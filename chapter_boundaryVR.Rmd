...
horner2016_mean_within <- mean(c(aidansData1$accuracy[aidansData1$Context == 'within'], 
                                 aidansData2$accuracy[aidansData2$Context == 'within']))

horner2016_SD_within   <- mean(c(aidansData1$SD[aidansData1$Context == 'within'], 
                                 aidansData2$SD[aidansData2$Context == 'within']))

horner2016_mean_across <- mean(c(aidansData1$accuracy[aidansData1$Context == 'across'], 
                                 aidansData2$accuracy[aidansData2$Context == 'across']))

horner2016_SD_across   <- mean(c(aidansData1$SD[aidansData1$Context == 'across'], 
                                 aidansData2$SD[aidansData2$Context == 'across']))
```
...
# Load data
exp1a_demographics <- read.table("data/Exp1/batch1/exp1a_demographics.csv", header = TRUE, sep = ',')
...
# Add room information to analyse across trials separately
...
# Transform values and variables factor
agg_order_b1_roomInfo$trans_acc <- arcsine_transform(agg_order_b1_roomInfo$acc)
agg_order_b1_roomInfo$id        <- as.factor(agg_order_b1_roomInfo$id)
agg_order_b1_roomInfo$roomType  <- as.factor(agg_order_b1_roomInfo$roomType)
agg_order_b1_roomInfo$boundary  <- as.factor(agg_order_b1_roomInfo$boundary)
```
...
plt1 <- ggplot(agg_order_b1, aes(x = boundary, y = acc, group = Condition, colour = Condition)) + 
  ...
  ## Data from Horner 2016
  geom_point(aes(x = 1.3, y = horner2016_mean_across), colour = "blue", shape = 15, size = 5, alpha = 0.1) 
...
```{r exp1_batch1_test1}
### Difference between across trials
# Get values
val1 <- agg_order_b1_roomInfo[agg_order_b1_roomInfo$context == 'across' & agg_order_b1_roomInfo$roomType == 'M-room', 'acc']
val2 <- agg_order_b1_roomInfo[agg_order_b1_roomInfo$context == 'across' & agg_order_b1_roomInfo$roomType == 'O-room', 'acc']
...
d1   <- signif(mean(val1 - val2)/sd(val1 - val2), 3)

# Test
bf01_1 <- ttestBF(val1, val2, paired = TRUE)
bf01_1 <- reportBF(1/bf01_1, digits, 'signif')
...
str3 <- mean_SD_str2(agg_order_overall_b1$acc, 1, digits, 'signif')

bf01_2 <- ttestBF(agg_order_overall_b1$trans_acc - arcsine_transform(1/3), nullInterval = c(-Inf, 0))
bf01_2 <- reportBF(1/bf01_2[2], digits, 'signif')

### Across versus M-room
val1  <- agg_order_b1[agg_order_b1$Condition == 'across', 'acc']
val2  <- agg_order_b1[agg_order_b1$boundary  == 'within' & agg_order_b1$Condition  == 'M-room', 'acc']

# Create report strings
str4 <- mean_SD_str2(val1, 1, digits, 'signif')
str5 <- mean_SD_str2(val2, 1, digits, 'signif')

# Transform values
val1 <- arcsine_transform(val1)
val2 <- arcsine_transform(val2)
d2   <- signif(mean(val1 - val2)/sd(val1 - val2), 3)


# Test
bf01_3 <- ttestBF(val1, val2, paired = TRUE, nullInterval = c(-Inf, 0))
bf01_3 <- reportBF(1/bf01_3[1], digits, 'signif')

### Across versus O-room
val1  <- agg_order_b1[agg_order_b1$Condition == 'across', 'acc']
val2  <- agg_order_b1[agg_order_b1$boundary  == 'within' & agg_order_b1$Condition  == 'O-room', 'acc']

# Create report strings
str6 <- mean_SD_str2(val1, 1, digits, 'signif')
str7 <- mean_SD_str2(val2, 1, digits, 'signif')

# Transform values
val1 <- arcsine_transform(val1)
val2 <- arcsine_transform(val2)
d3   <- signif(mean(val1 - val2)/sd(val1 - val2), 3)


# Test
bf01_4 <- ttestBF(val1, val2, paired = TRUE, nullInterval = c(-Inf, 0))
bf01_4 <- reportBF(1/bf01_4[1], digits, 'signif')

# Room question
str8 <- mean_SD_str2(agg_room_b1$acc, 1, digits, 'signif')

bf01_5 <- ttestBF(agg_room_b1$trans_acc - arcsine_transform(0.5), nullInterval = c(-Inf, 0))
bf01_5 <- reportBF(1/bf01_5[2], digits, 'signif')

# Table question
str9 <- mean_SD_str2(agg_table_b1$acc, 1, digits, 'signif')

bf10_6 <- ttestBF(agg_table_b1$trans_acc - arcsine_transform(0.5), nullInterval = c(-Inf, 0))
bf10_6 <- reportBF(bf10_6[2], digits, 'signif')
```

There was no conclusive evidence that memory performance in the across room condition differed as a function of whether the cue object was in an M, `r str1`, or O-room, `r str2`, BF_01 = `r bf01_1`, *d* = `r d1` (see Figure x), so that this condition was collapsed across rooms for further analysis. 

There was moderate evidence that temporal order memory was not above chance (i.e., averaged across within and across conditions; `r str3`), BF_01 = `r bf01_2`. This might be why there was also no evidence for a boundary effect - across, `r str4` ,  vs. within, `r str5`, - for either M rooms, BF_01 = `r bf01_3`, *d* = `r d2`, or O-rooms, BF_01 = `r bf01_4`, *d* = `r d3` - across, `r str6` ,  vs. within, `r str7`.

There was evidence that participants could not remember in the room type in which an object was presented, BF_01 = `r bf01_5`, `r str8`. The evidence for whether they could remember the table on which an object was presented was anecdotal only, BF_10 = `r bf10_6`, `r str9`.


```{r exp1_batch1_across_split}
# Run anova
anova1 <- anovaBF(trans_acc ~ roomType*boundary + id, 
                  whichRandom = 'id', 
                  whichModels = 'top', 
                  data = agg_order_b1_roomInfo, 
                  progress = FALSE)

bf01_1 <- reportBF(anova1[1], digits, 'signif')
bf01_2 <- reportBF(anova1[2], digits, 'signif')
bf01_3 <- reportBF(anova1[3], digits, 'signif')
```

Additionally, I submitted the data to a 2 x 2 repeated measure anova with room type (O vs. M room) and boundary (across vs. within) as factors. This uncovered that there is no evidence for a main effect of room type, BF01 = `r bf01_3`, for a main effect of boundary, BF01 =
`r bf01_2` and for the interaction term, BF01 = `r bf01_1`. 
...
Another notable concern was that memory performance was not above chance, which might have been another reason why I did not find the boundary effect. 
...
# Load data
exp1b_demographics <- read.table("data/Exp1/batch2/exp1b_demographics.csv", header = TRUE, sep = ',')
...
# Transform values and variables factor
agg_order_b2_roomInfo$trans_acc <- arcsine_transform(agg_order_b2_roomInfo$acc)
agg_order_b2_roomInfo$id        <- as.factor(agg_order_b2_roomInfo$id)
agg_order_b2_roomInfo$roomType  <- as.factor(agg_order_b2_roomInfo$roomType)
agg_order_b2_roomInfo$boundary  <- as.factor(agg_order_b2_roomInfo$boundary)
```
...
### Difference between across trials
# Get values
val1 <- agg_order_b2_roomInfo[agg_order_b2_roomInfo$context == 'across' & agg_order_b2_roomInfo$roomType == 'M-room', 'acc']
val2 <- agg_order_b2_roomInfo[agg_order_b2_roomInfo$context == 'across' & agg_order_b2_roomInfo$roomType == 'O-room', 'acc']
...
str3 <- mean_SD_str2(agg_order_overall_b2$acc, 1, digits, 'signif')

bf01_2 <- ttestBF(agg_order_overall_b2$trans_acc - arcsine_transform(1/3), nullInterval = c(-Inf, 0))
bf01_2 <- reportBF(1/bf01_2[2], digits, 'signif')

### Across versus M-room
val1  <- agg_order_b2[agg_order_b2$Condition == 'across', 'acc']
val2  <- agg_order_b2[agg_order_b2$boundary  == 'within' & agg_order_b2$Condition  == 'M-room', 'acc']
...
### Across versus O-room
val1  <- agg_order_b2[agg_order_b2$Condition == 'across', 'acc']
val2  <- agg_order_b2[agg_order_b2$boundary  == 'within' & agg_order_b2$Condition  == 'O-room', 'acc']
...
# Room question
str8 <- mean_SD_str2(agg_room_b2$acc, 1, digits, 'signif')

bf01_5 <- ttestBF(agg_room_b2$trans_acc - arcsine_transform(0.5), nullInterval = c(-Inf, 0))
bf01_5 <- reportBF(1/bf01_5[2], digits, 'signif')

# Table question
str9 <- mean_SD_str2(agg_table_b2$acc, 1, digits, 'signif')

bf10_6 <- ttestBF(agg_table_b2$trans_acc - arcsine_transform(0.5), nullInterval = c(-Inf, 0))
bf10_6 <- reportBF(bf10_6[2], digits, 'signif')
```

There was again weak evidence that memory performance was not above chance, BF01 = `r bf01_2`, `r str3`, for the temporal memory question. Furthermore, I did not find a boundary effect for M-rooms for accuracy, across: `r str4`, within: `r str5`, BF01 = `r bf01_3`, *d* = `r d2`, and I did not find an effect for O-rooms for accuracy, across: `r str6`, within: `r str7`, BF01= `r bf01_4`,  *d* = `r d3`. 

Participants also did not show above chance performance for remembering in which room type a cue object was presented in, BF01 = `r bf01_5`, `r str8`. However, there was stronger evidence that participants did remember on which table type a cue object was presented, BF10 = `r bf10_6`, `r str9`.


```{r exp1_batch2_across_split}
# Run anova
anova1 <- anovaBF(trans_acc ~ roomType*boundary + id, 
                  whichRandom = 'id', 
                  whichModels = 'top', 
                  data = agg_order_b2_roomInfo, 
                  progress = FALSE)
...
# Load data
exp1c_demographics <- read.table("data/Exp1/batch3/exp1c_demographics.csv", header = TRUE, sep = ',')
...
df_order_b3_roomInfo$roomType  <- df_room_b3$corr_resp
df_order_b3_roomInfo$roomType  <- ifelse(df_order_b3_roomInfo$roomType == 1, 'O-room', 'M-room')
agg_order_b3_roomInfo          <- ddply(df_order_b3_roomInfo, c('id', 'context', 'roomType'), summarise, acc = mean(accuracy), rt = mean(rt))
...
# Transform values and variables factor
agg_order_b3_roomInfo$trans_acc <- arcsine_transform(agg_order_b3_roomInfo$acc)
agg_order_b3_roomInfo$id        <- as.factor(agg_order_b3_roomInfo$id)
agg_order_b3_roomInfo$roomType  <- as.factor(agg_order_b3_roomInfo$roomType)
agg_order_b3_roomInfo$boundary  <- as.factor(agg_order_b3_roomInfo$boundary)
```
...
### Difference between across trials
# Get values
val1 <- agg_order_b3_roomInfo[agg_order_b3_roomInfo$context == 'across' & agg_order_b3_roomInfo$roomType == 'M-room', 'acc']
val2 <- agg_order_b3_roomInfo[agg_order_b3_roomInfo$context == 'across' & agg_order_b3_roomInfo$roomType == 'O-room', 'acc']
...
str3 <- mean_SD_str2(agg_order_overall_b3$acc, 1, digits, 'signif')

bf10_2 <- ttestBF(agg_order_overall_b3$trans_acc - arcsine_transform(1/3), nullInterval = c(-Inf, 0))
bf10_2 <- reportBF(bf10_2[2], digits, 'signif')

### Across versus M-room
val1  <- agg_order_b3[agg_order_b3$Condition == 'across', 'acc']
val2  <- agg_order_b3[agg_order_b3$boundary  == 'within' & agg_order_b3$Condition  == 'M-room', 'acc']
...
# Test
bf10_3 <- ttestBF(val1, val2, paired = TRUE, nullInterval = c(-Inf, 0))
bf10_3 <- reportBF(bf10_3, digits, 'signif')

### Across versus O-room
val1  <- agg_order_b3[agg_order_b3$Condition == 'across', 'acc']
val2  <- agg_order_b3[agg_order_b3$boundary  == 'within' & agg_order_b3$Condition  == 'O-room', 'acc']
...
# Room question
str8 <- mean_SD_str2(agg_room_b3$acc, 1, digits, 'signif')

bf01_5 <- ttestBF(agg_room_b3$trans_acc - arcsine_transform(0.5), nullInterval = c(-Inf, 0))
bf01_5 <- reportBF(1/bf01_5[2], digits, 'signif')

# Table question
str9 <- mean_SD_str2(agg_table_b3$acc, 1, digits, 'signif')

bf10_6 <- ttestBF(agg_table_b3$trans_acc - arcsine_transform(0.5), nullInterval = c(-Inf, 0))
bf10_6 <- reportBF(bf10_6[2], digits, 'signif')
```

This time there was strong evidence that memory performance was above chance  for the temporal memory question, BF10 = `r bf10_2`, `r str3`. While, I found a boundary effect for M-rooms  for accuracy, across: `r str4`, within: `r str5`, BF10 = `r bf10_3`, *d* = `r d2`, but I did not find an effect for O-rooms for accuracy, across: `r str6`, within: `r str7`, BF01 = `r bf01_4`, *d* = `r d3`. 
...
# Test
val1 <- agg_order_collasp_b3[agg_order_collasp_b3$context2 == 'across', 3]
val2 <- agg_order_collasp_b3[agg_order_collasp_b3$context2 == 'within', 3]
...
# Effect size
diffScores <- val2 - val1
d1         <- signif(mean(diffScores)/sd(diffScores), digits)


# test
test1 <- ttestBF(val1, val2,  paired = TRUE)
test1 <- reportBF(test1, digits, 'signif')
```

Interestingly, the difference between condition (across vs. within) collapsed across room type was still large, across: `r str1`, within: `r str2`, BF10 = `r test1`, d = `r d1`. 

```{r exp1_batch3_across_split}
# Run anova
anova1 <- anovaBF(trans_acc ~ roomType*boundary + id, 
                  whichRandom = 'id', 
                  whichModels = 'top', 
                  data = agg_order_b3_roomInfo, 
                  progress = FALSE)
...
Like before, I submitted the data to a 2 x 2 repeated measure anova with room type (O vs. M room) and boundary (across vs. within) as factors. This uncovered that there is no evidence for a main effect of room type, BF01 = `r bf01_3`, for a main effect of boundary, BF01 =
`r bf01_2` and for the interaction term, BF01 = `r bf01_1`. 

Participants again did not show above chance performance for remembering in which room type a cue object was presented in, BF01 = `r bf01_5`, `r str8`. However, there was strong evidence that participants did remember on which table type a cue object was presented, BF10 = `r bf10_6`, `r str9`.
...
table11 <- as.data.frame(table(da_sim_results_agg$band)/nIter)
...
table12 <- as.data.frame(table(da_sim_results_agg$band)/nIter)
...
```{r exp2_demographics}
# Load data
exp2_demographics <- read.table("data/Exp2/exp2_demographics.csv", header = TRUE, sep = ',')
...
```{r exp2_test1}
# Boundary effect for block1
# Get values
val1 <- agg_order_exp2_sub1$acc[agg_order_exp2_sub1$context == 'across']
val2 <- agg_order_exp2_sub1$acc[agg_order_exp2_sub1$context == 'within']
...
# test
bf01_1 <- ttestBF(val2, val1, paired = TRUE, nullInterval = c(-Inf, 0))
bf01_1 <- reportBF(1/bf01_1[2], digits, 'signif')
...
# Get values
val1 <- agg2_order_exp2$acc[agg2_order_exp2$context == 'across']
val2 <- agg2_order_exp2$acc[agg2_order_exp2$context == 'within']
...
# Effect size
diffScores <- val2 - val1
d2         <- signif(mean(diffScores)/sd(diffScores), digits)
...
# test
bf01_2 <- ttestBF(val2, val1, paired = TRUE, nullInterval = c(-Inf, 0))
bf01_2 <- reportBF(1/bf01_2[2], digits, 'signif')
...
bf01_3   <- reportBF(1/anova1[5])
max_bf01 <- max(as.vector(anova1))

# Which question has better memory
agg_order_exp2_question <-ddply(agg_order_exp2, c('worker_id', 'questionType'), summarise, acc = mean(acc))
val1 <- agg_order_exp2_question[agg_order_exp2_question$questionType == 'before', 'acc']
val2 <- agg_order_exp2_question[agg_order_exp2_question$questionType == 'after', 'acc']

str5 <- mean_SD_str2(val1, 1, digits, 'signif')
str6 <- mean_SD_str2(val2, 1, digits, 'signif')
...
# Overall test
bf10_4 <- ttestBF(agg_order_exp2_overall$trans_acc - arcsine_transform(1/3), nullInterval = c(-Inf, 0))
bf10_4 <- reportBF(bf10_4[2])

str7 <- mean_SD_str2(agg_order_exp2_overall$acc, 1, digits, 'signif')
```

Data collection terminated at N = 16, all of whom did the O-room condition, at which the BF for the null exceeded the criterion that there is no boundary effect for the first block for accuracy with O-rooms, across: `r str1`, within: `r str2`, BF_01 = `r bf01_1`, *d* = `r d1`. This result also held true when collapsing across both blocks, across: `r str3`, within: `r str4`, BF_01 = `r bf01_2`, *d* = `r d2`.

The planned ANOVA with boundary-type, question-type and block did not produce strong evidence for main effects or interactions, apart for the main effect of question type, BF_10 = `r bf01_3` with performance for the after question, `r str6`, being higher than before question, `r str5`. Overall, memory performance was low, but above chance, BF_10 = `r bf10_4`, `r str7`.
...
```{r exp3_demographics}
# Load data
exp3_demographics <- read.table("data/Exp3/exp3_demographics.csv", header = TRUE, sep = ',')
...
# Overall test
bf10_1 <- ttestBF(agg_order_exp3_overall$trans_acc - arcsine_transform(1/3), nullInterval = c(-Inf, 0))
bf10_1 <- reportBF(bf10_1[2], digits, 'signif')
...
### Boundary effect
# Get values
val1 <- agg_order_exp3$acc[agg_order_exp3$context == 'across']
val2 <- agg_order_exp3$acc[agg_order_exp3$context == 'within']

str2 <- mean_SD_str2(val1, 1, digits, 'signif')
str3 <- mean_SD_str2(val2, 1, digits, 'signif')
...
Mean accuracy did increase relative to Experiment 2, up to `r str1`, which was clearly above chance BF_10 = `r bf10_1`. However, there was no evidence for a boundary effect, in fact, some evidence against such an effect, across: `r str2`, within: `r str3` BF_01 = `r bf01_2`, *d* = `r d1`. 
