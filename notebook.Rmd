...
grid.arrange(grobs = list(gplot1, blank, legend, gplot2, gplot3, gplot4, blank),
             newpage = FALSE,
             top =  expression("Bayesian sequential design analysis with max sample size of 38 (d"[true]*" = 0.00)"),
             layout_matrix = lay,
             widths = c(12, 3, 2))
```
...
grid.arrange(grobs = list(gplot1, blank, legend, gplot2, gplot3, gplot4, blank),
             newpage = FALSE,
             top =  expression("Bayesian sequential design analysis with max sample size of 38 (d"[true]*" = 0.44)"),
             layout_matrix = lay,
             widths = c(12, 3, 2))
```
...
grid.arrange(grobs = list(gplot1, blank, legend, gplot2, gplot3, gplot4, blank),
             newpage = FALSE,
             top = expression("Bayesian sequential design analysis with max sample size of 38 (d"[true]*" = 0.78)"),
             layout_matrix = lay,
             widths = c(12, 3, 2))
```
...
As can be seen in the table above expect for the third simulation, the evidence rate is not very high and around 60%. Luckily, however the rate of misleading evidence is low throughout (<= 0.03). We will go ahead despite the relatively low evidence rate considering that we might not need a sample size of 36 for the m-shape room, so that we can use the remaining resources for the open-plane room. 
...
for(i in 1:n){
  ...
  # Create response data.frame if number of responses is above zero
  if(num_keyPress[i] > 0){
...
kable(df_video_exp2_intervalls_agg)
```
...
for(i in 1:n){
  ...
  tempDF$rt <- as.numeric(as.character(tempDF$rt))
  ...
  if(temporalOrder1$questionOrder == 0){
  ...
  # Direction of question
  if(temporalOrder2$questionOrder == 0){
  ...
  # Create or bind to data.frame
  if(i == 1){
    df_order_b3    <- rbind(temporalOrder1, temporalOrder2)
    df_order_b3$id <- i
  } else {
    temporalOrder1$id <- i
    df_order_b3      <- rbind(df_order_b3, rbind(temporalOrder1, temporalOrder2))
  }
}
...
bla <- paste(strwrap(text, 120), collapse = "\n")
printText2rmarkdown(bla)
```