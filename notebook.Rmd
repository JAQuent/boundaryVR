...
load("~/boundaryVR/data/Exp1/batch0/trialData_video2.RData")
trialData_video2 <- trialData_video2[order(trialData_video2$objNum),]

# Load all data
folder         <- 'memoryTask/'
prefix         <- "~/boundaryVR/data/Exp1/batch0/"
...
nQuestions     <- 13
prefix         <- '~/boundaryVR/data/Exp1/batch2/debrief/'
allFiles       <- list.files(paste(prefix, sep = ''))
allFiles_paths <- paste(prefix, allFiles, sep = '')
...
```{r}
df_order_table_exp2 <- ddply(df_order_exp2_dist, c('id', 'context', 'questionType', 'foil1_sameTable', 'foil2_sameTable'), 
                             summarise,
                             acc   = mean(accuracy))
...
ggplot(df_order_table_exp2, aes(x = sameTable, y = trans_acc, fill = sameTable)) +
  facet_grid(questionType~context) +
...
Average transformed (overall) accuracy is treated as an outlier if it's above/below 2 median absolute deviations from the median. This let to the exclusion of `r excluded` % of the data. 
...
```{r fig14, fig.height = 27, fig.width = 10}
for(i in 1:nQuestions){
  tempSubset    <- debriefData[debriefData$id == q_id[i],]
  tempDF        <- data.frame(table(tempSubset$answerString))
  names(tempDF) <- c('Answer', 'Freq')
